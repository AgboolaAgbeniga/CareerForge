...
const SecuritySettings: React.FC = () => {
    ...
    const fetchProfile = async () => {
        try {
            const data = await apiClient.auth.getProfile() as any;
            setProfile(data.user);
        } catch (err: any) {
            setError(err.message || 'Failed to fetch security profile');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDisable2FA = async (e: React.FormEvent) => {
        ...
        try {
            await apiClient.auth.disable2FA({ password, code: code || undefined });
            ...
            setError(err.message || 'Failed to disable 2FA. Please check your credentials.');
        } finally {
    ...
    const handleRegenerateCodes = async () => {
        ...
        try {
            const data = await apiClient.auth.regenerateBackupCodes({ password: pass }) as any;
