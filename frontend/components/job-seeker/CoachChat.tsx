...
export default function CoachChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: 'assistant', content: "Hi! I'm your CareerForge AI Coach. How can I help you land your next role today?" }
  ]);
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        // apiClient unwraps the { success: true, data: ... } standard format 
        // so it returns the data array directly if formatted properly.
        const history: any = await apiClient.get('/api/ai/coach/history');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await apiClient.post('/api/ai/coach/message', {
          messages: [
  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="fixed bottom-6 right-6 p-4 bg-indigo-600 text-white rounded-full shadow-2xl hover:bg-indigo-700 transition-all hover:scale-105 z-50 flex items-center justify-center group"
      >
  ...
  return (
    <div className={clsx(
      "fixed right-6 bottom-6 flex flex-col bg-slate-900 border border-slate-800 shadow-2xl rounded-2xl overflow-hidden z-50 transition-all duration-300 ease-in-out",
      isMinimized ? "w-80 h-16" : "w-96 h-[600px] max-h-[80vh]"
    )}>
      {/* Header */}
      <div className="flex items-center justify-between p-4 bg-slate-800 border-b border-slate-700 cursor-pointer" onClick={() => setIsMinimized(!isMinimized)}>
        <div className="flex items-center gap-3">
          <div className="p-2 bg-indigo-500/20 text-indigo-400 rounded-lg">
            <Bot size={20} />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-white">Career Coach</h3>
            <p className="text-xs text-slate-400">Powered by NVIDIA NIM</p>
          </div>
        </div>
          ...
          <div className="p-4 bg-slate-800 border-t border-slate-700">
            <form onSubmit={handleSubmit} className="flex gap-2">
              <input 
                ...
                className="flex-1 bg-slate-900 text-sm text-white placeholder-slate-500 rounded-full px-4 py-2 border border-slate-700 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
              />
              <button 
                type="submit"
