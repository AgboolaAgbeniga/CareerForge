...
try {
  ...
  logger.warn('⚠️ Environment validation failed during build:', error);
...
const getSwaggerServers = () => {
  ...
  if (isDevelopment) {
    ...
    // Use environment variable for production URL, fallback to Render detection
    const productionUrl = process.env.BASE_URL || process.env.RENDER_EXTERNAL_URL || 'https://careerforge-backend.onrender.com';
    return [
      {
        url: productionUrl,
...
const swaggerOptions = {
  ...
  apis: ['./src/**/*.ts', './src/*.ts', './dist/**/*.js', './dist/*.js'], // Path to the API docs
};
