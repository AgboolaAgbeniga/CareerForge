...
class ResumeParser:
    ...
    def __init__(self):
        logger.info("Initializing Resume Parser (Lazy Loading enabled)...")
    ...
    def ner_pipeline(self):
        if self._ner_pipeline is None:
            from shared.utils import get_ner_pipeline
            self._ner_pipeline = get_ner_pipeline()
        return self._ner_pipeline

    @property
    def embedder(self):
        if self._embedder is None:
            from shared.utils import get_embedder
    ...
    def layout_model(self):
        if self._layout_model is None:
            try:
                ...
                self._layout_model = lp.Detectron2LayoutModel('lp://efficientdet')
            except Exception as e:
                logger.warning(f"Layout parser not available: {e}")
                self._layout_model = False # Use False as a "checked but failed" marker
        return self._layout_model if self._layout_model is not False else None
